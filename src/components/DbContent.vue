<template>
    <div class="content">
        <List
          class="list"
          :items="peerTeachers"
          @select-row="updateCurrentPt"
          #default="slotProps"
        >
            <span>{{slotProps.item.toString()}}</span>
            <button 
                class="row-btn btn-remove"
                @click="removePt(slotProps.index)">Remove</button>
        </List>
        <List
          class="list"
          :items="labs"
        ...
        >
            ...
            <button 
                ...
                @click="assignLab(slotProps.index)">Add</button>
        </List>
        <List
          class="list"
          :items="this.currentAssignments"
          #default="slotProps"
        >
            <span>{{slotProps.item.toString()}}</span>
            <button 
                class="row-btn btn-remove"
                @click="unassignLab(slotProps.index)">Remove</button>
        </List>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        conflictWithAppts(appt1, appt2) {
            ...
            return daysConflict && appt1.startTime <= appt2.endTime && appt2.startTime <= appt1.endTime;
        },
        ...
        getLabIndex(id) {
            let low = 0, high = this.labs.length-1;
    ...
    computed: {
        /*
        compatibleLabs() {
            if(!("assignments" in this.currentPt))
                return Array(this.labs.length).fill(false);
            
            let compatible = [];

            for(let lab of this.labs) {
                if(this.conflictWithPt(this.currentPt, lab) || this.currentPt.assignments.includes(lab.id)) {
        ...
        */
        currentAssignments() {
            if(this.currentPt.assignments === undefined)
                return [];
